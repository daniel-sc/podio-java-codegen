...
public class CodeGenerator {
	...
	public void generateCode(Application app) {
		...
		try {
			...
			// original Item:
			JMember item = addMember(jc, "originalItem", jCodeModel.ref(Item.class), "Stores the original item, as retrieved by java-podio api.", jCodeModel);
			...
			JMember podioId = addMember(jc, "PodioId", jCodeModel.ref(Integer.class), "This represents the internal Podio id of the item.", jCodeModel);
			JMember podioRevision = addMember(jc, "PodioRevision", jCodeModel.ref(Integer.class), "This represents the internal Podio revision of the item.", jCodeModel);
	...
	private JClass getType(PodioType type, ApplicationField f) {
		...
		switch (type) {
			...
			case CATEGORY_SINGLE:
				String name = createValidJavaTypeName(f.getConfiguration().getLabel());
	...
	private JExpression createGetFieldValue(PodioType type, JVar jVar, JClass javaType) {
		switch (type) {
			...
			case CATEGORY_SINGLE:
				JExpression podioId = JExpr.direct("(Integer) ((Map<String, ?>) " + jVar.name() + ".getValues().get(0).get(\"value\")).get(\"id\")");
