...
public class PodioDate implements Serializable {
	...
	if (end != null && !end.equals("")) {
	    setEndDate(true);
	    setEnd(PODIO_DATE_TIME_FORMATTER.get().parse(end));
	...
	if (isStartDate()) {
	    dateHashMap.put("start",
		    isStartTime() ? PODIO_DATE_TIME_FORMATTER.get().format(start)
			    : (PODIO_DATE_FORMATTER.get().format(start) + " 00:00:00"));
	    dateHashMap.put("start_date", PODIO_DATE_FORMATTER.get().format(start));
	    dateHashMap.put("start_time", isStartTime() ? PODIO_TIME_FORMATTER.get().format(start) : null);
	}
	if (isEndDate()) {
	    dateHashMap.put("end",
		    isEndTime() ? PODIO_DATE_TIME_FORMATTER.get().format(end)
			    : (PODIO_DATE_FORMATTER.get().format(end) + " 00:00:00"));
	    dateHashMap.put("end_date", PODIO_DATE_FORMATTER.get().format(end));
	    dateHashMap.put("end_time", isEndTime() ? PODIO_TIME_FORMATTER.get().format(end) : null);
	}
