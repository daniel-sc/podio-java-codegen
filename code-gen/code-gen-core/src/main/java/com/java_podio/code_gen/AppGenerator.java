...
public class AppGenerator {
	...
	public JDefinedClass getAppClass(Application app) throws JClassAlreadyExistsException {
		...
		for (ApplicationField f : app.getFields()) {
			...
			jcase.body().invoke(field.getSetter()).arg(createGetFieldValue(type, setValuesFromItemForEachField.var(), javaType));
			jcase.body()._break();
	...
	public JMethod _setValue() throws JClassAlreadyExistsException {
		...
		setValuesFromItemForEachField = _setValue.body().forEach(jCodeModel.ref(FieldValuesView.class), "field", setValuesFromItemParam.invoke("getFields"));
		setValuesFromItemSwitch = setValuesFromItemForEachField.body()._switch(setValuesFromItemForEachField.var().invoke("getId"));
	...
	public JMethod _getItemCreate() {
		...
		itemCreateResult = _getItemCreate.body().decl(jCodeModel.ref(ItemCreate.class), "result", JExpr._super().invoke("getItemCreate"));
		_itemCreateFieldValues = _getItemCreate.body().decl(jCodeModel.ref(List.class).narrow(FieldValuesUpdate.class), "fieldValuesList", itemCreateResult.invoke("getFields"));
	...
	private JExpression createGetCurrencyFieldValue(JVar jVar) {
		return JExpr._new(jCodeModel.ref(PodioCurrency.class)).arg(createGetDoubleFieldValue(jVar)).arg(createGetStringFieldValue(jVar, "currency", jCodeModel));
	}
